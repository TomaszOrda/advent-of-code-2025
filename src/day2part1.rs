...
fn next_tandem_root(n: u64) -> u64{
    if !number_length(n).is_multiple_of(2){
        return 10_u64.pow(n.ilog10().div_ceil(2) )
    }
...
fn sum_of_tandems(min: u64, max: u64) -> u64{
    if min > max{
        ...
        min + sum_of_tandems(next_tandem(min), max)
    }
}

fn next_tandem(tandem:u64)->u64{
    let tandem_root = 10_u64.pow(tandem.ilog10().div_ceil(2));
    if (tandem + 1).ilog10() > tandem.ilog10(){
        return tandem_root * (tandem_root*10 + 1)
    }
    if !(tandem.ilog10() + 1).is_multiple_of(2){
        return tandem_root * (tandem_root*10 + 1)
    }
    tandem + (tandem_root + 1)
